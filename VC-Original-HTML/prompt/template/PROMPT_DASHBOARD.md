...
```javascript
// Vanilla JS 기반의 배열 데이터로 관리합니다.
const dashboardData = [
  {
    groupName: "가이드 및 파운데이션",
    items: [
      {
        no: "01",
        path: "Colors (색상)",
        type: "Page", // 'Page' | 'Popup'
        url: "/guide/colors.html",
        date: "2026-08-01",
...
> **⚠️ [주의 사항]**: 4가지 진행 상태(총 갯수, 완료, 리뷰, 대기) UI를 임의로 큰 박스나 Card 컴포넌트 형태로 거대하게 만들지 마십시오. 작은 텍스트와 숫자 위주의 미니멀하고 컴팩트한 레이아웃(Flex Row 형태)을 유지해야 합니다.
- **숫자 강조**: 폰트 크기(`30px`, `weight: 500`, `letter-spacing: -0.6px`)를 사용하여 간결하게 배치합니다.
...
- **데이터 행 (`item_row`)**: 마우스 오버 시 미세한 배경색 변화(`rgba(15, 23, 42, 0.025)`)를 주어 현재 읽고 있는 행을 강조합니다. URL 링크는 포인트 컬러 (`#0294de`)를 적용하고 오버 시 밑줄이 생기게 합니다.

### 2.4. 상태 뱃지 컬러 매핑 (Status Badges)
- **Published (완료)**: 긍정의 의미인 Emerald/Green 계열 (`color: #059669`, `background: #ecfdf5`)
